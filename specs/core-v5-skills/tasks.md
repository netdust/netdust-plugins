...
- [x] T04 — wp-security's NTDST section and wp-plan-requirements' drift example on v5 [Tier B]  (files: skills/wp-security/SKILL.md, skills/wp-plan-requirements/SKILL.md)
  Satisfies: FR-5
...
- [x] T05 — evals: retired-symbols.sh, 7 cases rewritten + 4 new, runner executed, REPORT rewritten [Tier A]  (files: evals/retired-symbols.sh, evals/behavioral-lessons.json, evals/REPORT.md, evals/outputs/correctness-results.json)
...
- [x] T06 — netdust-wp 1.0.0: plugin.json, marketplace catalog, README line [Tier B]  (files: .claude-plugin/plugin.json, ../../.claude-plugin/marketplace.json, README.md)
  Satisfies: FR-7, SC-6
...
- [x] T07 — re-anchor on core-shape T13 and on the 5.2.0 tag (widened 2026-09-02: core reached 5.1.0 / 5.1.1 / 5.2.0 while the branch waited; the anchor, the chain's grouping methods, the meta hooks + before-state arities, the 5.1.1 reads and the README rows the 5.0.0 pass missed all landed here; core's tags carry no `v` prefix) [Tier B]  (files: skills/ntdst-framework/SKILL.md, skills/ntdst-framework/references/traps.md, evals/REPORT.md)
  Satisfies: FR-8, SC-7
