---
name: statamic-mcp
description: Use when interacting with Statamic content (entries, terms, globals, structures, assets, users) or system state programmatically — prefer the Statamic MCP router tools over file edits where possible. Triggers on intent to create/update/list Statamic entries, edit globals, scan blueprints, manage assets, run system ops, manage users/roles. Activates on keywords statamic-entries, statamic-terms, statamic-globals, statamic-structures, statamic-assets, statamic-users, statamic-system, statamic-blueprints, statamic-discovery, statamic-schema, content:read, content:write, scoped API token. Symptoms include needing to bulk-update entries, fetching a blueprint shape, updating a global value, creating a taxonomy term, querying system health. Requires Statamic v6+ and laravel/mcp v0.6+. Reference skill.
---
