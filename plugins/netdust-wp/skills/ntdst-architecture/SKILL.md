...
description: >
  ...
  `ntdst-core`, `NTDST_Bootstrap`, `ntdst_router`, `ntdst_response`,
  `NTDST_Theme`. Also use when reviewing NTDST code for framework drift, or when
  a framework call is missing and you need to know whether this project's copy of
  ntdst-core has it.
---
...
`register_taxonomy()` directly, 4/13 register raw `wp_ajax_*` — every one of them documented,
accurately, the whole time. If a rule is grep-decidable it belongs in the gate; this skill keeps the
judgment calls, and `references/anti-patterns.md` keeps the worked wrong/right pairs the drift
reviewer keys off.
...
| `Theme::module()` DSL, `Theme::register()`, `::taxonomy()`, `::apiAction()` | 7 of 8 copies — **retired in daan** |
...
HEAD on Data, Response, Endpoints, Theme and Metabox — port from it, don't assume from it.
...
| An admin settings page, a form/data flow, a YOOtheme surface | `ntdst-patterns`' `golden-paths/` |
| A standalone plugin (own container, not on ntdst-core) | `references/plugin-scaffold.md` |
...
|---|---|
| Register a CPT | `ntdst_data()->register($name, $config)` |
| Register a taxonomy | the `taxonomies` key inside that same `register()` config |
...
| Bind a hook, a template path, a mixin | `$theme->on()` / `->filter()` / `->templatePath()` / `->mixin()` |
| Send mail | `ntdst_mail()` |
| Log an event | `ntdst_log($channel)` |

### If you are porting from an older copy

daan retired four `Theme` wrappers with **no shim** — a call to one is a fatal, and a listener on a
retired filter name is silently inert. Reach for the owner instead:

| write this | instead of |
|---|---|
| `ntdst_data()->register($type, $config)` | `$theme->register(...)` |
| the `taxonomies` key on that config | `$theme->taxonomy(...)` |
| `ntdst_api_action($action, $handler, $opts)` | `$theme->apiAction(...)` |
| `$theme->mixin(...)` | the `$theme->module(...)` DSL |
| `NTDST_Template_Loader::page()` / `::pageData()` | `NTDST_Response::page()` / `::pageData()` / `::toRestResponse()` |
| `ntdst_service_{slug}_config` / `_enabled`, overrides at `config['services']['overrides'][$slug]` | `netdust_{slug}_config` / `_enabled` |
| `admin/MetaboxGenerator.php`, `admin/RelationField.php` | `api/MetaboxGenerator.php`, `services/RelationField.php` |

`ntdst_page_data()` **survives** — it now calls the template loader directly instead of hopping
through a Response forwarder.
...
| `references/architecture.md` | PHP standards, size limits, design patterns, hook naming, helper index |
| `references/anti-patterns.md` | The drift canon — worked wrong/right pairs, the `ntdst-drift-reviewer` checklist |
| `references/plugin-scaffold.md` | Standalone plugin structure (own container, not on ntdst-core) |
...
| Template output | **Response / template loader**, never raw `echo` | Inside a service |
| Standalone plugin (not on ntdst-core) | **Plugin scaffold** | `references/plugin-scaffold.md` |
...
| does this service boot, and with what config? | `core/Bootstrap.php` | `ntdst_service_{slug}_enabled` → DB option `ntdst_service_{slug}`; config via `ntdst_service_{slug}_config`, fed from `config['services']['overrides'][$slug]` |
...
| what capability floors an API action? | `ntdst_api_floor_cap()` in `api/Endpoints.php` | type-derived from the post type object, resolved at dispatch, **fail-closed** |
| may this actor write a model's fields? | the metabox save method in `admin/` | read access to its error notices uses the same predicate, so read ⊆ write by construction |
...
| where does a template resolve from? | `NTDST_Template_Loader` | one live registry, read at resolution time, `realpath`-confined |
...
- **A file is not a class.** `api/Response.php` declares **two** classes — `NTDST_Response` and
  `NTDST_Template_Loader`. A file-per-class assumption produced a wrong plan premise (a helper was
  called dead; a live template consumes it every render). Grep for the symbol, not the filename.
...
- **Two config-filter surfaces exist and do not meet.** Bootstrap fires `ntdst_service_{slug}_config`
  ...
  **does not disable it** — Bootstrap only checks `ntdst_service_{slug}_enabled`. Pick deliberately;
  document which one the service honors. On a fork still using a retired filter name this fails
  **open**: a deny filter that no longer matches lets the service boot.
...
- **Hook names are passed through verbatim, and must be.** `Theme::on()`/`filter()` deliberately do
  not `sanitize_key()` — that strips `/` and `.`, so binding a framework `ntdst/…` filter through the
  facade registered a name nothing fires, and it lowercased `myPlugin_Action` into a different hook.
...
| *"The other site's ntdst-core has this method, so ours does."* | Twelve-plus forks, 40% apart, no upstream. Confirm the symbol in *this* project's copy before you call it. |
| *"The framework must have a helper for this."* | Sometimes it does not, and a defended raw-WP idiom is correct — pre-query interception on `parse_request` is a real example. Defend it explicitly with an `ntdst-allow` reason; don't force a helper that doesn't fit, and don't pretend the gap isn't one. |
...
whose name doesn't start `ntdst_service_`; a `$theme->register(` / `->taxonomy(` / `->apiAction(` /
`->module(` call; reaching for the nearest sibling file as your template.
...
- **Route data through a repository**, output through the Response/template loader, URLs through Router.
...
- **Carry a `drift pre-check clean` acceptance line** — `bin/drift-check.py` green, or every allow
