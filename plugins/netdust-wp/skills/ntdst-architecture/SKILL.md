...
- **Use config filter** pattern: `apply_filters('ntdst_service_{slug}_config', $defaults)`. The name is **framework-owned and slug-keyed**, not project-prefixed — Bootstrap hangs `plugin-config.php`'s `services.overrides.{slug}` on exactly that hook, so any other name silently receives no override. Same for `ntdst_service_{slug}_enabled`. A project's own prefix belongs on its DOMAIN events (`stride/registration/created`), not on these two
