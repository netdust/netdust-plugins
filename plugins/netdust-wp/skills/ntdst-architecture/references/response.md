...
| `toRestResponse()` | `WP_REST_Response` | **Non-exiting** JSON representation for the NTDST_Rest dispatch path — same payload as `json()` + the stored status, but returns instead of `exit`ing. |
