...
**This file replaces ten API-reference files and two code templates.** They carried ~4,100 lines of
...
## The tree, so you know which file owns what

```
ntdst-core/
├── admin/     MetaboxGenerator.php, RelationField.php   ← admin UI + the write-authorization gate
├── api/       Data.php, Endpoints.php, Response.php     ← data + transport
├── core/      Bootstrap.php, Container.php, Router.php, Theme.php,
│              SectorRegistry.php, ServiceInterface.php, LogLevel.php
├── services/  Logger.php, Mailer.php                    ← infrastructure services
└── assets/    js/ntdst-api.js, css/admin-toolkit.css
```

Three directories, three jobs: `api/` is data and transport, `services/` is infrastructure, `admin/`
is admin UI. A metabox generator belongs to none of the first two, which is why it has its own home.
...
- **The filters Bootstrap actually fires are `ntdst_service_{slug}_enabled` and
  `ntdst_service_{slug}_config`**, and the config filter is fed from
  `config['services']['overrides'][$slug]`. The older `netdust_{slug}_*` names are **gone with no
  shim** — a listener on a retired name is silently inert.
...
- **The rename fails OPEN in the wrong direction.** `_enabled` is normally used as a *deny* filter
  (`__return_false`). A fork still listening on the retired `netdust_{slug}_enabled` name therefore
  does not disable the service — it boots. Check the filter name before trusting a kill switch.
- **The service slug is derived from the class name, and the derivation is lossy.** It collapses
  acronyms — `AdminUIService` → `admin_ui` — but stripping `Service` strips **every** occurrence, so
  `ServiceService` yields an empty slug. It is also many-to-one: two classes can collide on one slug
  and silently share an enable filter and a DB option.
- **`metadata()['name']` does not win.** Through the real discovery flow the enable-check warms the
  ...
  Renaming a service in `metadata()` renames neither its filters nor its DB option.
- **Two config-filter surfaces exist and do not meet.** Bootstrap fires `ntdst_service_{slug}_config`.
  Services also conventionally apply their own `{project}_{slug}_config`. A service that reads only
  its project-prefixed filter is **invisible to `config['services']['overrides']`**, and
  `add_filter('{project}_{slug}_enabled', '__return_false')` **disables nothing**. Pick deliberately;
  document which surface the service honors.
- **Namespaced services are not auto-discovered.** Discovery matches `*Service.php` at the discovery
  root plus enabled sector directories only. A namespaced class in any other subdirectory must be
  listed in the bootstrap config or it silently never loads.
...
- **Wrong priority band is a real defect class.** Roughly: 1–5 critical infrastructure, 6–9 core
  framework and data models, 10–14 standard features, 15–19 content, 20–29 UI, 30+ optional. A UI
  feature at priority 3 is sitting in infrastructure territory and will boot before what it depends on.
...
## Data & CPT registration — `api/Data.php`

**Never call `register_post_type()` or `register_taxonomy()` directly, anywhere.** Bypassing the data
layer's wrapper breaks four things at once: field definitions no longer drive validation, metabox and
query building; the repository contract (`ntdst_data()->get(POST_TYPE)`) only works for
wrapper-registered types; the post-type constant stops being reusable; and the type stops being
discoverable by convention.

Taxonomies are declared **with the model**, in the `taxonomies` key of the same `register()` config,
and registered only after the post type itself succeeds. (`NTDST_Data_Manager::registerTaxonomy()`
exists as the direct door for the rare case a taxonomy genuinely has no owning model.) The
`Theme::taxonomy()` wrapper that used to hold the defaults is retired — a taxonomy outlives a theme
switch, so it was never Theme's job — and its defaults were lifted verbatim, so changing them now
changes every taxonomy that relied on the wrapper.

A CPT may also live in a dedicated `*CPT` class that owns its registration and exposes the
`POST_TYPE` constant. **That class is not a service** — it is a static config holder called from the
owning service's `init()`.

→ Registration defaults, return shapes, the capability trap and the fail-closed rules: `ntdst-data`.
...
- **`ntdst_router()` fires at `template_include` — too late to rewrite query vars.** Pre-query
  interception is a legitimate raw `parse_request` hook; that is not drift. Annotate it.
...
**This one file declares two classes** — `NTDST_Response` and `NTDST_Template_Loader`. A
file-per-class assumption here produced a wrong plan premise (a helper was called dead; a live
template consumes it every render). Grep the symbol, not the filename.

**Decides:** the JSON envelope, and where a template resolves from.

- **`NTDST_Response` is the single owner of the success/error envelope.** Every output surface routes
  through it.
- **Two error shapes coexist deliberately — do not "unify" them.** One shape serves the JS API
  ...
  been attempted.
- **`NTDST_Template_Loader` is the home for page and page-data resolution** — `::page()` renders,
  `::pageData()` reads back, `::addPath()` registers a directory. `ntdst_page_data()` is the global
  a template calls, and it **survives**: it now calls the loader directly. The `NTDST_Response`
  forwarders onto these (`page()`, `pageData()`, `toRestResponse()`) were **deleted** — a forwarder
  is a second surface that drifts independently, and two of these drifted twice in one week.
...
- **`template()` defers (returns self, for routing), `render()` outputs and exits, and `html()`
  returns a string** for emails and AJAX fragments. Choosing the wrong one is the single most common
  Response mistake.
...
- **`render()` exits the request.** Correct for a route; a live bug inside an admin render callback,
  where it kills every panel after yours. In an admin callback, `include` the template directly.
...
- **Register with `ntdst_api_action($action, $handler, $opts)`.** The underlying hook is an ordinary
  WordPress filter, so a raw `add_filter('ntdst/api_data/…')` works — and forfeits the declared
  capability floor, the public-allowlist entry, and the dispatch-time gate.
...
→ The `$opts` keys, the fail-closed floor, and the worked slice: `ntdst-data`.
...
A REST registrar (`ntdst_router()->rest()`) and a CORS policy (`NTDST_Cors_Policy`) exist in **one**
site's copy and were never ported — they are **absent from daan's `core/Router.php`**. Do not assume
...
- **A per-route permission callback is required; there is no default.** A missing or non-callable
  ...
  callable. `'permission_callback' => '__return_true'` is the canonical security bug and is gated.
...
## Admin UI — `admin/MetaboxGenerator.php`, `admin/RelationField.php`

**Decides:** how a model's edit screen renders, and **who may write its fields**.

- **Name this convergence point by class and method, never by path.** The write-authorization
  invariant survived a whole-directory `git mv` completely untouched for exactly that reason, while
  every path-shaped reference in the tree needed updating.
...
- The relation autocomplete lives here as the non-public `relation_search` action, gated on two cheap
  questions: is the requested type a **declared relation target** (an allow-list derived from the
  registered schemas), and does the caller hold that type's own `edit_others_posts`? A type nobody
...
The wrappers that violated this were **retired**, and there is no shim:

| retired | its owner now |
|---|---|
| `$theme->register()` | `ntdst_data()->register(…)` |
| `$theme->taxonomy()` | the `taxonomies` key on that same `register()` config |
| `$theme->apiAction()` | the global `ntdst_api_action($action, $handler, $opts)` |
| `$theme->module()` DSL | retired outright, not relocated — `mixin()` supersedes it |

Most other copies still carry all four.
...
a facade over five unrelated subsystems — not chaining. Retained: `on`, `filter`, `when`,
`templatePath`, the routing shortcuts `single`/`page`/`archive`, `mixin`, `__call`, and
config/assets.
...
- **Do not run hook names through `sanitize_key()`.** It strips `/` and `.`, so binding one of the
  framework's own namespaced filters (`ntdst/…`) through the facade registered a name nothing ever
  fires, and it lowercased `myPlugin_Action` into a different hook. This was a live bug; `on()` and
  `filter()` now pass the name through verbatim.
- **Owners stay reachable at one hop through mixins.** `$theme->data()`, `->router()`, `->response()`,
  `->log()`, `->mail()` resolve through `__call()`, not as named methods. The rule governs what earns
  a *named method*, not what is reachable.
- The module concept is retired; mixins supersede it. A method-injection mixin cannot override a
  method that already exists on `NTDST_Theme`.
...
The scaffold itself is in `references/plugin-scaffold.md`.
