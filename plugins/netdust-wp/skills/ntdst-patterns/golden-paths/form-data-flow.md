...
Governing reference: **`netdust-wp:wp-security`** (the four pillars + exact sanitize/escape/authorize functions), **`ntdst-architecture/references/framework-map.md`** (the `ntdst/api_data` contract). This doc does not restate the pillar tables — it shows where each pillar lands.
...
- Governing references: `netdust-wp:wp-security` (four pillars + functions), `ntdst-architecture/references/framework-map.md` (the `ntdst/api_data` contract).
