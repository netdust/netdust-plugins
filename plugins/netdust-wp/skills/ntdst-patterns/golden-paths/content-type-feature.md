...
Governing reference: **`ntdst-architecture/golden-paths/feature-service.md`** (service lifecycle), **`ntdst-data`'s `golden-paths/model-and-api-action.md`** (Data API + repository contract). This doc does not restate those rules — it shows the shape they produce.
...
> See `anti-patterns.md` → *Manual template_include* and `references/framework-map.md` → Router. The raw filter is gated as `manual-template-include`.
...
- Governing references: `ntdst-architecture/golden-paths/feature-service.md`, `ntdst-architecture/references/framework-map.md` (Router), `ntdst-data/golden-paths/model-and-api-action.md`.
