...
- **Cross-origin (CORS)**: WP core's default reflects any `Origin` **and** sets `Access-Control-Allow-Credentials: true` (the reflection+credentials anti-pattern), so any origin can read authenticated responses. **ntdst-core 4.x ships no CORS handling at all** — there is no `cors` route option, and `NTDST_Rest` refuses any route carrying one. Do not hand-roll `Access-Control-*` headers or a `rest_pre_serve_request` hook as a one-off either; this is an open gap that needs a framework answer, and the requirements plus two paid-for traps are written down in `ntdst-architecture/references/rest-cors.md` → *The CORS gap*. An anonymous WP nonce authenticates nothing for a cookie-less cross-origin caller — never use the `ntdst/api_data` AJAX path for cross-origin.
