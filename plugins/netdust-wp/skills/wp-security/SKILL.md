...
- **REST**: `permission_callback` is **required** on every route. `__return_true` is the canonical security bug. New REST routes register through `ntdst_router()->rest()` (`NTDST_Rest_Registrar`), which makes `permission` a required option with **no default** — a route missing it is not registered at all (structural enforcement, not a review catch). See `ntdst-architecture/references/framework-map.md` → *Cross-origin REST*. **Check the registrar exists in this project's `ntdst-core` copy first** — it was built on one site and never ported; elsewhere you register routes yourself and the required-permission rule is yours to enforce.
- **Cross-origin (CORS)**: never hand-roll `Access-Control-*` headers or a `rest_pre_serve_request` hook — WP core's default reflects any `Origin` **and** sets `Access-Control-Allow-Credentials: true` (the reflection+credentials anti-pattern). Use `NTDST_Cors_Policy` (exact-origin allow-list, prio-20 override, credentials stripped, `Origin: null` never allowed) passed as the route's `cors` option. Never `*`; never substring-match an origin. An anonymous WP nonce authenticates nothing for a cookie-less cross-origin caller — do not use the `ntdst/api_data` AJAX path for cross-origin.
