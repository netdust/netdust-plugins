...
(`api/Data.php` — `register()`, `registerTaxonomy()`; `api/Endpoints.php` — `ntdst_api_action()`,
`ntdst_api_floor_cap()`; `admin/MetaboxGenerator.php`), and against a live consumer,
...
## The field vocabulary, and the `$opts` vocabulary

Two enumerations worth knowing exist because you cannot guess a member of either.

**Field `type` values** — `text`, `textarea`, `email`, `url`, `html`, `content`, `wysiwyg`, `int` /
`integer`, `float` / `double`, `bool` / `boolean`, `date`, `select` (needs `options`), `array`,
`json`, `relation` (needs `post_type`), `gallery`, `repeater` (needs `fields`). Each drives a real
sanitizer and a real admin control. **An unknown type throws at registration** — it used to fall
through to `sanitize_text_field` silently, which is how a `wysiwig` typo quietly stripped a field's
markup with nothing failing. Read the exception message for the authoritative accepted set, and read
`api/Data.php` for which sanitizer each one maps to; that mapping is the part that changes.

Per-field validation options: `required`, `min`, `max`, and a `validate` callback.

**`ntdst_api_action()` `$opts` keys** — `public` (bool), `cap_type` (a post type; the type-derived
fail-closed floor), `capability` (a literal capability, retained only because it was reconciled from
the retired `Theme::apiAction()` wrapper and it keeps that wrapper's fail-**open**-on-empty
semantics), and `priority`. Prefer `cap_type`.
