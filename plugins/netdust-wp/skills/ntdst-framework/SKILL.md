...
**README 5.0.0 rows reconciled at the 5.2.0 re-anchor** (absent from the groups above): `NTDST_Bootstrap::config()` · `ntdst_container()` · `ntdst_inline()` · `sanitize_field()` · `NTDST_Service_Meta` · `NTDST_Rest::surface()` · `'before_dispatch' => fn($r) => …` · `GET /ntdst/v1/download?action=X` · the filters `ntdst/api/allowed_origins`, `ntdst/api/public_actions`, `ntdst/api/rate_limit/{action}`, `ntdst/api/rate_window/{action}`, `ntdst/api_data/{action}`, `ntdst/api_download/{action}` · `NTDST_Template_Loader::getCustomPaths()` · `NTDST_Pages::commitOk()` · `NTDST_Pages::handleTemplateInclude()` · `NTDST_Pages::preventRedirectForRoutes()` · `NTDST_Pages::redirect($url)` · `NTDST_Pages::renderResponse()` · `NTDST_Pages::resolveRouteResult()` · the hook names `ntdst_model_create_after`, `ntdst_model_update_after`, `ntdst_model_delete_after` (their replacements are the lifecycle hooks below).
...
- **`array`/`json` string leaves keep their newlines (5.1.1)** — the registry runs them through `sanitize_textarea_field()`, so a consumer's own textarea sanitizer composes instead of being silently overwritten by a stricter rule. **`repeater()` decodes the stored JSON-string shape (5.1.1)** — a legacy/raw write or the `register_post_meta()` re-save no longer empties every row; garbage still answers `[]`.
...
**Model lifecycle hooks** are `ntdst/model/creating` · `created` · `updating` · `updated` · `deleting` · `deleted`, plus `registering` and `registered` — same arguments as the retired names, and no shim. **Since 5.2.0 three of them carry an APPENDED before-state argument** — `updated` a fourth (`['post' => [column => previous], 'meta' => [field => ['exists' => bool, 'value' => mixed]]]`, exactly the fields the caller wrote), `deleting` and `deleted` a third (`['post' => WP_Post, 'meta' => formatted fields]`, the only place the row survives after the delete). WordPress hands a listener only as many arguments as it registered for, so a listener that wants the snapshot says so in `accepted_args`.

**Direct meta writes have their own hooks since 5.2.0.** `updateMeta()` and `updateMetaBatch()` fire `ntdst/model/meta_updated` `($post_type, $id, array $data, array $previous)` on the success path only — `$data` is the sanitized values actually written (unprefixed field keys), `$previous` maps the same keys to `['exists' => bool, 'value' => mixed]`; a batch fires ONE event carrying every written key. `deleteMeta()` fires `ntdst/model/meta_deleted` `($post_type, $id, $key, array $previous)` only when something was actually deleted. Before 5.2.0 these paths were invisible to the model hooks; an audit listener on `updated` never saw a status flip written through `updateMeta()`.

**One query API: the chain.** `ntdst_data($type)->where(…)->withMeta()->get()`. There is no formatted-posts helper and no term helper — terms are `wp_set_object_terms()` and `wp_get_object_terms()`, WordPress's own. **The grouping methods (5.1.0)** are where the retired `orWhere()` went: `whereIn($field, $values)` and `whereNotIn($field, $values)` (`'ID'` sets `post__in` / `post__not_in`), `whereMissing($field)` (`NOT EXISTS` on the prefixed key), and `whereGroup($relation, fn($q) => …)` — ONE nested `meta_query` clause with its own `AND`/`OR`; the callback receives a child builder of the same model, so keys are prefixed and `scope()` resolves inside the group; groups nest; only the child's META clauses are read (a `whereTax`, `limit`, `orderBy` or core-field `where` inside the callback is ignored); a relation other than `AND`/`OR` throws `InvalidArgumentException`; an empty group is a no-op.
