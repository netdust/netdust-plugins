...
- **Review gates and shake-out.** Every implementer dispatch names `netdust-wp:ntdst-framework` + `wp-testing`. (Stefan, 2026-09-03: six approved tasks had built plain WordPress on the framework because neither happened.) Drift review runs at the branch review and on the clusters the gate's `✓ [panel-hints] drift-panel:` line names — those whose `(files:)` touch `Services/`, `Handlers/`, `Repositories/` or `Modules/`; read the line, do not re-derive it. The cluster panel at FULL is `security-sentinel` + `invariant-auditor`, `ntdst-drift-reviewer` standing in where no `ARCHITECTURE-INVARIANTS.md` exists; the tier table lives in `netdust-agent:building`. (Re-ruled 2026-09-06, `specs/artifact-gate`: fifteen every-gate panel runs found one Critical, outside a panel.)
