...
- **Neither marker present → do NOT default to Codeception.** The gate layer is the target for any non-Stride project: adopt it from its single home, the `gate/` payload of github netdust/wp-starter (note that on an already-running project `wptests` is created by a post-start hook, so one `ddev restart` is needed before `test:int` works; README-testing §4c), or confirm with the user that the project is genuinely Stride-family before continuing to the legacy path.
