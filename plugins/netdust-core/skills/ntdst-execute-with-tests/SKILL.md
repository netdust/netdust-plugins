...
**Step 2.5 — Ground-truth the dependency surface before each dispatch (plan-freshness gate).** A written plan is a *hypothesis* about the code it integrates against; the source is truth. When the plan is more than a few days old, OR it integrates against another sub-phase's / module's code (calls its functions, names its enums, scopes, env vars, table columns, event payloads), the controller MUST — for the specific task about to be dispatched, AFTER the upstream skill is loaded (never as pre-flight before Step 1) — Read the actual exported signatures + types + enums of that task's named dependencies and reconcile them against the plan's code samples. Bake the verified-true signatures into the dispatch prompt and flag any drift inline so the implementer builds to reality, not to the stale sample. This is per-task, not whole-plan up front — verify each task's surface as you reach it.

Calibration (why this is a hard step, not advice): three consecutive Folio Phase-3 sub-phases hit plan-vs-source drift that this catch resolved — A (Zod house-style + migration columns), C.2 (an entire provider API — `continueWithToolResult`, injectable `AbortController`, non-existent error_reason enums, a FK-violating `system:` actor — none of which existed), C.3 (`recoverOrphanRuns({staleThresholdMs})` vs the plan's `recoverOrphanRuns(db)`, plus a contaminated Drizzle-generated migration). Every drift was caught at controller ground-truthing and corrected before/at dispatch. Skipping it ships the drift into the subagent, which then builds the wrong thing confidently. If reconciliation surfaces drift big enough to change the task's shape, correct the plan (a plan-correction commit) before dispatching. Reinforces the project-local `plan-server-source-audit` lesson, promoted here to a skill rule (2026-05-29 decision, Folio Phase 3 C.3 gate).
