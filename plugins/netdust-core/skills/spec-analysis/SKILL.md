...
```bash
python3 <netdust-core>/spec-kit/gate-check.py specs/<feature>
```
...
- missing/N/A threat model on a flagged surface → author it via `netdust-core:threat-modeling`, embed in the plan.
...
| `netdust-core:spec-authoring` | **UPSTREAM (Stage 0.5).** Its Security-relevant surfaces flags are what the threat-model cross-check keys on. |
...
| `netdust-core:threat-modeling` / `architecture-invariants` / `testing-workflow` | **REMEDIATION.** Where the checker fails, these author the missing gate. |
| `superpowers:subagent-driven-development` | **DOWNSTREAM (Stage 2).** Only runs once this gate is green. |
| `netdust-core:harnessed-development` | **SEQUENCER.** Inserts this as Stage 1.5 (Phase C edit). |
