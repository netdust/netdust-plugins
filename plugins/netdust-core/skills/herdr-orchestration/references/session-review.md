# Session-review watcher — operational reference

Loaded by `herdr-orchestration`'s `## Role — the session-review pane` when this
session IS the watcher. The skill carries the decisions; this file carries the
mechanics.
...
A proposal that touches a SKILL targets the MARKETPLACE SOURCE repo
(`~/Projects/netdust-plugins`) — never `~/.claude/plugins/cache/**`, which the
next `plugin update` erases. Name the source path even when you are running
somewhere else.
