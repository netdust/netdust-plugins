...
> Note: if you reached `/shakeout` via `netdust-core:harnessed-development`, its Stage 3 already ran this audit — you may skip Step 0. Running `/shakeout` standalone, this is the only place the audit fires, so do not skip it.

Use the Skill tool, pointed at the branch diff (Situation A — phase/spec-complete audit):

```
Skill("netdust-core:test-effectiveness")
```
