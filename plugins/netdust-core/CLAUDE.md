...
- **Cross-stack workflow** — `harnessed-development` is the single entry point: it sequences the full harness (design → **spec-authoring (Stage 0.5)** → plan + threat-modeling + architecture-invariants → **spec-analysis (Stage 1.5)** → execute + per-task testing-workflow + **standards-gate** + Step 2.5 → shake-out → finish) so no gate is skippable, and defers to the loaded stack sub-plugin for stack-specific tools. Supporting skills: `code-audit`, `shake-out`, `testing-workflow`, `test-effectiveness`. (`ntdst-execute-with-tests` was the execution-only predecessor — now deleted; `harnessed-development` fully absorbed it. The name resolves to nothing; older handoff docs that say "execute the plan" route to `harnessed-development` by its own triggers.) A `SubagentStop` hook (`hooks/subagent-stop.py`) backstops the testing gate (and the standards gate, where a linter is configured) by blocking subagent stops that edited code without running tests / the linter.
- **spec-kit graft** — `spec-kit/` + `/spec-kit-setup` install [github/spec-kit](https://github.com/github/spec-kit) per-project with gate-bearing override templates so its spec→plan→tasks output carries the harness gates. `constitution-bridge` generates the spec-kit constitution as a view over RULES/SOUL/invariants. `spec-kit/gate-check.py` mechanically verifies the gates landed. **Keystone: the handoff is `tasks.md` → Stage 2; `/speckit.implement` is never run.** See `docs/spec-kit-integration-adr.md`.
