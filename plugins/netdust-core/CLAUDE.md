...
- **Cross-stack workflow** — `code-audit`, `shake-out`, `testing-workflow`, `ntdst-execute-with-tests` (wraps superpowers:executing-plans + subagent-driven-development to enforce testing-workflow at task and phase boundaries). A `SubagentStop` hook (`hooks/subagent-stop.py`) backstops this by blocking subagent stops that edited code without invoking testing-workflow.
