...
- **2026-08-13**: agent 0.19.0 — spec-kit re-audit harvest: new `convergence` gate (`/converge`, code-vs-spec completeness → PROPOSED phase in tasks.md behind gate-check + the seam) + three semantic passes in planning Stage 1.5 (duplication, vague-adjective ambiguity, terminology drift). Marketplace catalog entry re-synced (had been left at 0.17 through the 0.18 release). Reminder for source-repo sessions: clones go stale — `git fetch` and re-anchor on origin/main BEFORE authoring; this session first built against a 0.17-era clone and had to re-land everything.
