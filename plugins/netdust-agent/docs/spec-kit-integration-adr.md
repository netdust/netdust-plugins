...
> `netdust-agent` plugin. The spec-kit graft was cherry-picked onto current `main` and
> **re-homed into `plugins/netdust-agent/`** (it feeds the execute spine, which `netdust-agent`
