...
Arm, disarm, or inspect the harness loop for one spec-kit feature. The loop is
the `loop-gate.py` Stop hook consuming `spec-kit/loop-check.py` (the ledger):
...
3. **Stage 1.5 green:** run `python3 <plugin>/spec-kit/gate-check.py <feature-dir>`
...
Run `python3 <plugin>/spec-kit/loop-check.py <feature-dir>` (feature dir from
