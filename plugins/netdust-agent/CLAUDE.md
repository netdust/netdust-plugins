...
- `spec-analysis` — gate (Stage 1.5): semantic cross-check (`/speckit.analyze` when the graft is present, otherwise by reading the artifacts — not machine-checked either way) + mechanical `gate-check.py` — the pre-execution barrier that machine-checks 1a/1b/1d/1f and needs no graft
