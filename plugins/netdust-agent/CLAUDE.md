...
One documented exception: the session-start hook's truncation warning names `/memory-audit` as a best-effort pointer to netdust-core's archive command (netdust-core is that command's canonical owner). It is not load-bearing — everything the harness actually gates on resolves within this plugin.
