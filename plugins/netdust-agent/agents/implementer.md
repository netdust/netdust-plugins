...
**5. Commit atomically.** Load the craft skill `versioning-with-git` — one commit per task, with the Test-evidence / STATUS in the commit body. On `split`, your implementation commit is separate from the test-author's test commit, so git shows the test predated the code. On `solo`, your single commit carries both the test and the implementation — the evidence block states this openly (`Contract test author: self — solo mode`); there is no second commit to separate them from.
...
**`solo` task — reproduce exactly:**
   ...
   - Deferral: Risk this does NOT cover: <concurrency | adversarial-input |
     ...
     and the phase-close test-effectiveness audit, not here.
...
- Name what the tests do NOT cover in the Deferral line, keyed to the downstream gate that catches it (integration / `/code-review` / invariant-auditor / `/shakeout`). Honest deferral is part of the discipline, not a weakness. On a `solo` task, the deferral line is also where you name that the independent check moved downstream to the cluster review gate + phase-close test-effectiveness audit — that is not a gap to hide, it's the stated trade.
