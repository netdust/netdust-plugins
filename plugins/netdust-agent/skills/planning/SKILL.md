...
| `tasks.md` | a `Unit test:` continuation line per task — or an `Integration test:` line | the contract, or `no unit test: Tier B, <reason>`, or `Integration test: <contract>` where the risk is only integration-testable (WP hooks/CPT/DB — the real-WP behaviour Brain Monkey cannot express); either form satisfies, for any tier, and text after `Integration test:` is always a contract, never a waiver. Partial presence → **FAIL** naming the bare tasks; a **Tier A** task waiving it → **FAIL**; none at all → WARN |
...
**1d. Task-shaping gate (one gate, three facets — absorbs the former 1f and 1h).** The content gates above decide what the plan must contain; this gate shapes the task list those contents break into. The labels 1d/1f/1h survive in the templates and `gate-check.py`; treat them here as one gate:
  ...
  - **(1d) Test expectations.** Per `testing-workflow`: every task gets its tier marker — a Tier-A task carries a "Unit test: [what the RED-first test must assert]" line (including the denial path for a guard) — or, where the risk is only integration-testable, an `Integration test: <contract>` line whose contract must name the real-WP behaviour the risk lives in that Brain Monkey cannot express (hooks/CPT registration/DB); every Tier-A obligation carries over, denial path named in the contract. A Tier-B task carries `no unit test: Tier B, <reason>` (there is no waiver form inside `Integration test:`). Every phase gets an "Integration gate: [what to verify across tasks]" line. Integration-contract tasks are never `[P]` — they share one `wptests` DB whose schema is reinstalled per run; serialize them. A plan without these is not ready to execute.
