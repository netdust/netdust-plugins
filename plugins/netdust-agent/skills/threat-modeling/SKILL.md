...
- **Traverse-clause bypass (correct-at-fetch, wide-at-serve):** the scoping check is applied where the data is *fetched* but NOT re-applied where it is *broadcast, listed, serialized, or live-pushed* to a caller on a different (narrower) scope. The query is right; the consumer leaks. Three recurring manifestations: (a) a tenant-scoped event/feed delivered to a more-narrowly-scoped subscriber without re-filtering each row by the subscriber's reach (SSE, websocket, activity stream, notification fan-out, WP `admin-ajax`/REST list callbacks); (b) a "list X in tenant" path that narrows by a *service/agent* allow-list but not by the *human* caller's narrower grant, so a partially-scoped member sees the whole tenant; (c) the scope check living on ONE auth method or ONE route while a sibling path (a personal access token, an admin twin route, a WP capability-vs-nonce split, a null-scope/"all" sentinel row) skips it. **The tell:** the same "what may this actor see" decision is re-derived at N call sites instead of computed once — every extra site is where the next refactor silently diverges. The fix is structural, in the stack's own idiom (a shared query helper / a `map_meta_cap` filter / a blueprint-permission check / a policy method) — converge the visibility decision into the project's authorization convergence point and route every fetch/serve/broadcast surface through it. *(Worked example — Folio, CR-8/9/10/11: workspace-scoped SSE rows, ws-scoped run lists, and a role-change event all reached a project-only invitee because the per-user narrowing lived on one surface + one auth method; converging it into a single visibility helper closed all four.)*
...
| `planning` / `building` | **UPSTREAM + DOWNSTREAM.** `planning` FIRES this skill at gate 1a (by trigger list, or on an ad-hoc security diff in Class D, routed via `harnessed-development` intake). When `building` then executes the plan, the controller pre-flight verifies tasks include the mitigations; testing-workflow still gates per-task; the threat model gates at /code-review at sub-phase close. |
