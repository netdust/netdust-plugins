...
**1. Surface conflict, do not guess silently.** When the context is ambiguous, or two sources disagree (a `DECISIONS.md` entry contradicts a plan, the code contradicts `STATE.md`), do NOT pick one and proceed. This is exactly the harness intake's **"if you cannot classify, ask"** (`harnessed-development`) and `building`'s **Step 2.5 plan-freshness ground-truth** — re-verify against the code/the user before acting on a stale or conflicting premise. A silent guess on a conflicting premise is the failure mode this skill exists to prevent.
...
- **`harnessed-development` / `building`** — reach for this skill at session start / task switch; the intake's "if you cannot classify, ask" + building's Step 2.5 ground-truth is where "surface conflict not guess" lands.
