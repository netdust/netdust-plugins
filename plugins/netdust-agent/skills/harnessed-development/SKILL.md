...
| "I'll dispatch now and write the threat model / invariant when `/code-review` flags something" | That is the retrospective failure mode (1a BLOCKING). A threat model written for the fix documents pain already taken (calibration: `drop-workspace-retrofit`, told in full at 1a above). Write the section BEFORE the first dispatch. |
...
**Calibration data behind these rules** (all from Folio Phase 3; cross-harness index with slugs: `skills/_shared/calibrations.md`):
