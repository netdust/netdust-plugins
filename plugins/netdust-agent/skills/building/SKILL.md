...
|---|---|
| **A / B — plan-driven work** | The seam artifact, all three parts: **(1)** `tasks.md` exists for the feature; **(2)** `python3 <plugin>/bin/gate-check.py specs/<feature>` exits **0** — run it NOW, do not trust a transcript assertion (yours or `planning`'s); **(3)** your human partner has approved the plan. **Any part missing → REFUSE to start.** Name the missing part and route the work to `planning` (Class B additionally gets `planning`'s freshness review when the plan is stale or externally authored). **There is no variant of this ticket that accepts a stated checklist in place of exit 0.** `gate-check.py` depends on no external tooling — it reads `specs/<feature>/` directly, on every project. Run the script; a checklist an agent says it applied is the self-attestation the seam exists to refuse. If the feature dir isn't where the script can read it, that is the defect to fix, not to work around. A task carrying **no `Test-author:` line at all** defaults to **`split`**, never `solo`: see Step 2.1b. Note this now only reaches you on a `tasks.md` that states a `<!-- gate-check: legacy-artifact — <reason> -->` waiver — without one, a task list carrying no modes FAILs the gate and never crosses the seam. |
...
- **Reviewer independence is HALF-MECHANICAL.** Every reviewer persona's tools list omits `Edit`/`Write`, so a reviewer structurally cannot fix what it finds — that half is config, not prose. The other half — that the agent which wrote the code is not the one reviewing it — is sequencer-enforced: no hook can see authorship across two dispatches. Step 2.8 states the rule; treat it as load-bearing precisely because nothing will stop you.
...
**Step 2.8 — HALT at every review-gate marker (the cluster boundary is a hard stop).** When you reach a `── REVIEW GATE ──` / STOP marker in the plan (placed by `planning`'s task-shaping gate), OR the end of a phase's task group, you STOP. Commit the cluster's tasks, run `/integration` on that cluster's diff, then review — and do NOT begin the next task until that review is clear. The diff a reviewer holds must be one cluster (~3–4 tasks), never a whole long phase run flat. **The pull to "just keep going to the next task, I'll review at the end" is the exact failure the cluster rule exists to prevent** — it produces an un-bisectable mega-diff and lets the agent grade a large body of its own work in one pass. Treat the marker as non-negotiable as a failing test. If the plan you're executing is a long phase with NO such markers, that is a planning defect — add the markers (a plan-correction commit) before running past ~4 tasks. (Slug: `teardown-cluster`.)
  ...
  **Reviewer independence — the agent that wrote the code never reviews it.** The review is a DISPATCH to the reviewer personas, never a pass you do inline over your own diff. This is the review-stage twin of `<test_dev_split>`, and it fails the same way when skipped: **a self-review converges on the happy path.** The author already believes the denial path is handled — that belief is what produced the code — so a reviewer holding it asks the same questions and finds the same nothing. `doubting-decisions` states the principle for a plan decision ("the agent that just made a decision is the worst-placed to attack it"); it applies verbatim to a diff.
  ...
  - **The controller may orchestrate, never judge.** Dispatching the fan-out, collecting findings and sequencing the fixes is the controller's job. Reading your own cluster's diff and declaring it reviewed is not, even when the diff is small and you are confident — confidence in your own work is the disqualifying condition, not a licence.
  - **LIGHT tier reduces fan-out, never independence.** A doc/copy/config cluster gets ONE generalist `reviewer` — still a separate dispatch. There is no tier at which the author reviews.
  - **What is mechanical, and what is not.** Mechanical: every reviewer persona (`reviewer`, `security-sentinel`, `performance-oracle`, `invariant-auditor`, `code-simplicity-reviewer`, `shakeout-qa`) ships with **no `Edit`/`Write` in its tools list** — a finder can never quietly become the fixer, so findings must come back and be acted on deliberately. NOT mechanical: nothing hooks-enforces that the *implementing* agent stayed out of the reviewing seat. No hook sees who authored what across two dispatches — the same blind spot `<test_dev_split>` names for test authorship. This rule is the control, which is why it is written here instead of assumed.
...
| "There is a runner that can just execute the whole task list — I'll use that" | Any flat executor bypasses every Stage-2 gate — threat-model verify, per-task tiers, review-cluster HALT, the `subagent-stop` backstop. The handoff is `tasks.md` and THIS spine executes it, task by task, through the gates. |
| "The cluster is small and I know exactly what I changed — I'll review it myself and save a dispatch" | Knowing exactly what you changed is the disqualification. You would review the diff you meant to write, not the one you wrote, and the denial path you believe is handled is the one that never gets a second look. Dispatch the reviewers (Step 2.8). |
| "It's a LIGHT-tier doc cluster, a review dispatch is overkill" | LIGHT cuts the fan-out to one generalist `reviewer`; it never cuts independence to zero. There is no tier at which the author reviews. |
