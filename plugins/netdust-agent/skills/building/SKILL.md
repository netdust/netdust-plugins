...
**Class E — default `solo`; a security-boundary file is always Class D, never E.** A small tweak is one TDD cycle (no plan, no shake-out) with the mode read the same way: `solo` by default, where the controller may act as the implementer's own RED-author for a genuinely trivial inline change (record `Contract test author: self — Class E inline split, controller authored RED`). Anything touching a named security-boundary file is Class D, not E, and Class D is **always `split`** — its diff threat model (from `planning`'s 1a) names the contract the test-author asserts.
</test_dev_split>
...
3. Every code task's dispatch shape MATCHED its `Test-author:` mode from `tasks.md`, read by the controller and never re-decided by a dispatched agent: a `split` task ran as a `test-author → implementer` PAIR — an independent test-author authored the RED test and reported `## Test contract` + `RED_READY` in its own commit BEFORE the implementer, and the implementer greened that same test with `Weakened? NO`; a `solo` task ran as ONE `implementer` dispatch that authored its own RED-first test and greened it, recording `Contract test author: self — solo mode` and `Weakened? n/a — self-authored (solo mode)`. Every dispatch carried its matching verbatim addendum (A/B for split, C for solo). No `split` task's agent authored and greened its own contract test (except the recorded Class E inline exception, where the controller — not the coder — authored the RED), and no task's mode was decided by the agent executing it.
