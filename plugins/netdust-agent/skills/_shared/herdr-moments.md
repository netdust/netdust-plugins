...
| **Parallel dispatch** (`[P]` siblings, a contract-lane split pair, any second code-editing agent) | a worktree workspace per agent, unfocused, based on the production branch read from `site.yml` (devops owns the base — the `--base master` in herdr-orchestration's example is a framework-fix example, not the rule); result read from git in that worktree, never from the pane | sequential behaviour-lane work stays in the main pane as subagents — no topology for work that needs no isolation |
...
| **The screenshot yield** (`/shakeout` step 4) | one `shakeout` tab in the feature's workspace, unfocused, paging `specs/<feature>/shakeout/*.png` beside `shakeout.md`; then say the tab exists | not per flow — one tab for the run; never when `## Acceptance flows` is N/A, there is nothing to show |
| **A flow verb refuses on a dirty tree** (`make hotfix` mid-feature: production is broken and the feature is half-done) | a worktree workspace off `origin/<production branch>`, unfocused; hotfix there, `make gate` there if you want the result early, then hand the operator `make ship` — it runs the gate itself, so this is not required | when the feature work is at a natural stopping point — commit it and hotfix in this checkout. Never `make save` on unfinished work purely to unblock the hotfix |
